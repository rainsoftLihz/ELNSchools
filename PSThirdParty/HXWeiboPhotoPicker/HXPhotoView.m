...
- (void)deleteAllPhoto{
    NSInteger count = [self.manager afterSelectedArray].count;
    if (count == 0) {
        return;
    }
    for (NSInteger i = count - 1; i >= 0; i--) {
        [self deleteModelWithIndex:i];
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.cellRemoveDeleteBtn) {
        [cell deleteDeleteBtn];
    }
    cell.delegate = self;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == HXPhotoModelMediaTypeCamera) {
        ...
        JZTPhotoPreviewViewController *vc = [[JZTPhotoPreviewViewController alloc] init];
        vc.outside = NO;
