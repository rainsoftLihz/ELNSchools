...
@implementation HXPhotoManager(JZTExtented)

- (void)fetchOrigImagesCompletion:(void (^)(NSArray *))completion{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSMutableArray *tempArr = @[].mutableCopy;
        __block NSInteger count = self.afterSelectedArray.count;
        dispatch_block_t numJianBlock = [^{
            count--;
            if (count == 0) {
                if (completion) {
        ...
        for (HXPhotoModel *obj in self.afterSelectedArray) {
            if (obj.networkPhotoUrl) {
                numJianBlock();
            } else if (obj.asset) {
                PHAsset *asset = obj.asset;
                [[PHImageManager defaultManager]requestImageDataForAsset:asset options:0 resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
//                    UIImage *image  = [UIImage imageWithData:imageData];
                    UIImage *image2 = [UIImage imageWithData:imageData scale:3];
//                    if (imageData.length > 100) {
//                        image2 = [UIImage compressWithData:imageData width:ScreenWidth];
//                    }
//                    UIImage *image3 = [UIImage compressWithData:imageData width:ScreenWidth];
                    [tempArr addObject:image2];
                    numJianBlock();
                }];
            } else {
                [tempArr addObject:obj.thumbPhoto];
                numJianBlock();
            }
